...
-- ----------------------------------------------------------------------
-- 9. Обычный ANALYZE и CREATE INDEX пишут свежие relstats в pg_class.
--    После этого session-local fasttrun cache уже не имеет права
--    реинжектить старые значения поверх core-статистики.
-- ----------------------------------------------------------------------
CREATE TEMP TABLE t_survive_regular_analyze (id int);
INSERT INTO t_survive_regular_analyze SELECT generate_series(1, 10);
SELECT fasttrun_analyze('t_survive_regular_analyze');
INSERT INTO t_survive_regular_analyze SELECT generate_series(11, 100000);
ANALYZE t_survive_regular_analyze;

SELECT reltuples = 100000 AS pgclass_analyze_tuples
  FROM pg_class WHERE oid = 't_survive_regular_analyze'::regclass;

SELECT reltuples = 100000 AS fasttrun_analyze_tuples
  FROM fasttrun_relstats('t_survive_regular_analyze');

CREATE TEMP TABLE t_survive_create_index (id int);
INSERT INTO t_survive_create_index SELECT generate_series(1, 10);
SELECT fasttrun_analyze('t_survive_create_index');
INSERT INTO t_survive_create_index SELECT generate_series(11, 100000);
CREATE INDEX ON t_survive_create_index (id);

SELECT reltuples = 100000 AS pgclass_create_index_tuples
  FROM pg_class WHERE oid = 't_survive_create_index'::regclass;

SELECT reltuples = 100000 AS fasttrun_create_index_tuples
...
DROP TABLE t_survive_regular_analyze;
DROP TABLE t_survive_create_index;
