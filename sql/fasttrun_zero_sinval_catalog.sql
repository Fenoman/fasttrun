...
ALTER TABLE t_zsc ALTER COLUMN big SET STORAGE EXTERNAL;
INSERT INTO t_zsc
SELECT g, g % 50, string_agg(md5((g * 1000 + s)::text), ' ')
...
DECLARE
  ...
  toast_oid oid;
  toast_chunks0 bigint; toast_chunks_empty bigint; toast_chunks1 bigint;
  checksum0 text; checksum1 text;
BEGIN
  ...
  EXECUTE format('SELECT count(*) FROM %s', toast_oid::regclass)
    INTO toast_chunks0;
  SELECT md5(string_agg(md5(big), '' ORDER BY id)) INTO checksum0 FROM t_zsc;

  PERFORM fasttruncate('t_zsc');
  EXECUTE format('SELECT count(*) FROM %s', toast_oid::regclass)
    INTO toast_chunks_empty;
  INSERT INTO t_zsc
  SELECT g, g % 50, string_agg(md5((900000 + g * 1000 + s)::text), ' ')
  FROM generate_series(1, 200) g
  CROSS JOIN LATERAL generate_series(1, 450) s
  GROUP BY g;
  EXECUTE format('SELECT count(*) FROM %s', toast_oid::regclass)
    INTO toast_chunks1;
  SELECT md5(string_agg(md5(big), '' ORDER BY id)) INTO checksum1 FROM t_zsc;
  ...
  RAISE NOTICE 'external_toast_before=%', (toast_chunks0 > 0);
  RAISE NOTICE 'toast_empty_after_truncate=%', (toast_chunks_empty = 0);
  RAISE NOTICE 'toast_refill_different=%',
    (toast_chunks1 > 0 AND checksum1 <> checksum0);
END$$;
