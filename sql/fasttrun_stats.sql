...
--
-- ВАЖНО: собранная статистика переживает commit и живет до конца сеанса.
-- Транзакционный callback на commit снимает только привязку к счетчикам
-- транзакции, а на abort откатывает изменения этой транзакции. В BEGIN/COMMIT
-- обернуты те секции, где важна граница транзакции сама по себе.
--

/*
 * Поколоночный учет UPDATE работает только там, где библиотека предзагружена:
 * иначе операторы, начавшиеся до ее загрузки, остались невидимыми, и учет
 * выключается на весь сеанс. Набор поднимает предзагрузку для своей базы и
 * переподключается, а в конце возвращает все как было.
 */
...
-- Без статистики по колонкам планировщик берет DEFAULT_EQ_SEL=0.005
...
-- более точная оценка стоимости, а форма плана может не меняться
...
--    Хук должен заметить, что pgstat-счетчики изменились
...
--     BRIN передает хуку неинициализированный VariableStatData, поэтому
...
-- ----------------------------------------------------------------------
-- 8d. DDL нетронутой таблицы не должен скрывать ее ядерную статистику,
--     даже если кэши fasttrun уже созданы для другой таблицы.
-- ----------------------------------------------------------------------
DO $$
DECLARE ln text; est int; stat_count int;
BEGIN
  EXECUTE 'CREATE TEMP TABLE t_unmanaged_ddl_seed (id int)';
  EXECUTE 'INSERT INTO t_unmanaged_ddl_seed SELECT generate_series(1, 100)';
  PERFORM fasttrun_analyze('t_unmanaged_ddl_seed');

  EXECUTE 'CREATE TEMP TABLE t_unmanaged_index (id int, v int)';
  EXECUTE 'INSERT INTO t_unmanaged_index SELECT g, g FROM generate_series(1, 10000) g';
  EXECUTE 'ANALYZE t_unmanaged_index';
  EXECUTE 'CREATE INDEX ON t_unmanaged_index (id)';
  FOR ln IN EXECUTE 'EXPLAIN SELECT * FROM t_unmanaged_index WHERE v = 4242' LOOP
    IF ln ~ 'on t_unmanaged_index' THEN
      est := substring(ln FROM 'rows=(\d+)')::int;
      EXIT;
    END IF;
  END LOOP;
  SELECT count(*) INTO stat_count FROM pg_statistic
   WHERE starelid = 't_unmanaged_index'::regclass;
  IF est IS DISTINCT FROM 1 OR stat_count <> 2 THEN
    RAISE EXCEPTION 'CREATE INDEX hid core stats: est=%, rows=%', est, stat_count;
  END IF;

  EXECUTE 'CREATE TEMP TABLE t_unmanaged_alter (id int, v int)';
  EXECUTE 'INSERT INTO t_unmanaged_alter SELECT g, g FROM generate_series(1, 10000) g';
  EXECUTE 'ANALYZE t_unmanaged_alter';
  EXECUTE 'ALTER TABLE t_unmanaged_alter SET (autovacuum_enabled = false)';
  est := NULL;
  FOR ln IN EXECUTE 'EXPLAIN SELECT * FROM t_unmanaged_alter WHERE v = 4242' LOOP
    IF ln ~ 'on t_unmanaged_alter' THEN
      est := substring(ln FROM 'rows=(\d+)')::int;
      EXIT;
    END IF;
  END LOOP;
  SELECT count(*) INTO stat_count FROM pg_statistic
   WHERE starelid = 't_unmanaged_alter'::regclass;
  IF est IS DISTINCT FROM 1 OR stat_count <> 2 THEN
    RAISE EXCEPTION 'ALTER TABLE hid core stats: est=%, rows=%', est, stat_count;
  END IF;
END$$;
\echo unmanaged_ddl_keeps_core_stats
DROP TABLE t_unmanaged_ddl_seed;
DROP TABLE t_unmanaged_index;
DROP TABLE t_unmanaged_alter;
...
--     Управляется порогом: если доля измененных строк
--     (insert+update+delete с момента предыдущего сбора, деленная на
...
--      отдает кешированную стату планировщику (как ядро PG между ANALYZE).
--      После 11a grp=1 - это все строки, так что план остается Seq Scan.
BEGIN;
SELECT fasttrun_analyze('t_refresh');
EXPLAIN (COSTS OFF) SELECT * FROM t_refresh WHERE grp = 1;
...
-- 11d. Порог = 1.0 отключает пересбор и задает максимальную толерантность:
...
UPDATE t_refresh SET grp = 999;     -- любой DML -> пересбор отключен
SELECT fasttrun_analyze('t_refresh');
...
--      rollback тот же below-threshold DML остается в пределах толерантности
...
--      продвигает pgstat counters в родительский subxact, а churn остается в
--      пределах толерантности, поэтому soft freshness отдает кешированную
--      стату.
--
--      High-cardinality таблица: редкий equality-предикат на кешированной
--      стате дает Index Scan (на defaults был бы Bitmap).
...
-- 11g. Targeted UPDATE концентрирует 10% строк на grp=1 - выше строгого порога
--      для high-cardinality колонки.  Cardinality-aware freshness прячет стату,
--      и planner уходит на defaults (Bitmap), а не строит Index Scan по
--      "редкому" grp=1, который на деле стал частым.  Защита сохраняется и
--      после COMMIT.
...
-- Targeted churn выше строгого порога high-card -> стата скрыта -> defaults.
EXPLAIN (COSTS OFF) SELECT * FROM t_commit_stale_stats WHERE grp = 1;
COMMIT;

-- После COMMIT защита сохраняется -> план остается на defaults.
EXPLAIN (COSTS OFF) SELECT * FROM t_commit_stale_stats WHERE grp = 1;
...
-- 11h. Cardinality-aware граница: для high-cardinality (unique) колонки порог
--      толерантности ужесточен (concentrated DML может раздуть "редкое"
--      значение в катастрофический Index Scan).  Мелкий churn ниже строгого
--      порога держит стату видимой (Index Scan), churn выше - прячет ее.
CREATE TEMP TABLE t_skew_below (id int, grp int);
INSERT INTO t_skew_below SELECT g, g FROM generate_series(1, 100000) g;
CREATE INDEX ON t_skew_below (grp);
BEGIN;
SELECT fasttrun_analyze('t_skew_below');
DELETE FROM t_skew_below WHERE id <= 3000;    -- 3% ниже строгого high-card порога -> видна
EXPLAIN (COSTS OFF) SELECT * FROM t_skew_below WHERE grp = 50000;
COMMIT;
DROP TABLE t_skew_below;

CREATE TEMP TABLE t_skew_above (id int, grp int);
INSERT INTO t_skew_above SELECT g, g FROM generate_series(1, 100000) g;
CREATE INDEX ON t_skew_above (grp);
BEGIN;
SELECT fasttrun_analyze('t_skew_above');
DELETE FROM t_skew_above WHERE id <= 10000;   -- 10% выше строгого high-card порога -> прячется
EXPLAIN (COSTS OFF) SELECT * FROM t_skew_above WHERE grp = 50000;
COMMIT;
DROP TABLE t_skew_above;

-- ----------------------------------------------------------------------
-- 12. Порог = 0 не должен запускать пересбор, если доля изменений
--     равна нулю.  Семантика порога: "0 = пересбор при любом DML",
--     а не "при каждом теплом попадании в кэш".
-- ----------------------------------------------------------------------
...
--     уровня, а не просто вытеснить ее.
-- ----------------------------------------------------------------------
...
--     Главное: после DML никакая устаревшая статистика не выдается.
-- ----------------------------------------------------------------------
...
-- После UPDATE: все еще оценки по умолчанию, никакой устаревшей
-- статистики
EXPLAIN (COSTS OFF) SELECT * FROM t_tc WHERE grp = 1;
COMMIT;
DROP TABLE t_tc;
...
--     оставаться несмещенным и ANALYZE-подобным.
--
--     Refresh path читает все heap-блоки. Выборка по части блоков даже
--     без early-stop bias не дает полной parity с ANALYZE для физически
--     упорядоченных или разреженных таблиц.
--
...
SELECT fasttrun_analyze('t_bias');
-- После пересбора: n_distinct ВСЕ ЕЩЕ должен быть высоким.
...
--
--     Путь пересбора в delta-hit ветке двигает baseline и на пустой
--     выборке. Если бы fasttrun_collect_and_store вызывался только при
--     sample_count > 0, после mass DELETE до пустоты baseline стоял бы на
--     месте, и каждый следующий fasttrun_analyze заново входил бы в путь
--     пересбора и платил за полный refresh, который снова возвращает ноль
--     строк.
--
--     Тут проверяем строго по структуре, а не по таймингам (тайминги
--     нестабильны на CI): каждый шаг должен соответствовать своей
--     ожидаемой четкой ветке.  План для grp=1 после mass DELETE
--     ожидаем как Seq Scan на пустой таблице (через дефолты - потому
--     что hook freshness check отвергает старую stats: pgstat
--     счетчики уже не совпадают со снимком от cold scan'а).  Главное:
...
--
--     Пересбор внутри подтранзакции продвигает stats baseline до
--     post-DML pgstat счетчиков, и subxact callback откатывает его вместе
--     с кэшем. Без этого после ROLLBACK TO SAVEPOINT pgstat счетчики
--     вернулись бы к pre-DML, baseline в analyze cache остался бы "из
--     будущего", и следующий fasttrun_analyze насчитал бы ненулевой
...
--     шел по дельта-математике (а не платил за полный cold scan), и
--     при этом еще и обновлял статистику колонок через полный reservoir
--     refresh.
--
...
-- здесь идет через delta-hit для rowcount + полный reservoir refresh
...
--      correlation в кэше становится ложной при неизменных DML-счетчиках,
--      freshness это не ловит.  Эвикция срабатывает до выполнения, а откат
...
--     index relation, и fasttrun должен публиковать тот же planner-visible
...
--
--     fasttrun регистрирует оба. Без (б) ширина уходила бы в
--     get_typavgwidth() с дефолтной шириной типа.  Для text это
--     32 байта независимо от реального содержимого, что ломает
...
--     ровно 19 байт ожидаем width <= 25 (Haas-Stokes дает ~19, а
--     get_typavgwidth() для text дает 32 - далеко за порогом).
-- ----------------------------------------------------------------------
...
-- Прямой запрос к плану через EXPLAIN.  Используем функцию-обертку,
...
--     значениями v in 2..1001.  С нашим путем через std_typanalyze
...
BEGIN
    ...
    IF rows_freq <> rows_rare THEN
        RAISE EXCEPTION 'Haas-Stokes дает разные оценки для v=1 (%) и v=500 (%): не должно быть разницы без MCV',
                        rows_freq, rows_rare;
    END IF;
...
--     а без histogram уйдет в DEFAULT_RANGE_INEQ_SEL = 0.5 -> ~50000.
-- ----------------------------------------------------------------------
...
-- Контр-тест: без typanalyze идет DEFAULT_RANGE_INEQ_SEL
BEGIN;
SET LOCAL fasttrun.use_typanalyze = off;
SELECT fasttrun_analyze('t_range');
DO $$
...
BEGIN
    ...
    -- Без histogram - DEFAULT_RANGE_INEQ_SEL x ndistinct, обычно >30000
...
--
--     Не сравниваем точные числа rows между ANALYZE и нашим путем
...
--     300 x default_statistics_target = 30000 при default DST=100).
--
--     Цели теста:
--       (1) -1 не ломает функционал - fasttrun_analyze не падает,
--           inspect_stats возвращает строки.
--       (2) на той же таблице -1 дает **точно тот же набор колонок**
...
--
--     pgstat-счетчики temp-таблицы обнуляются на границе транзакции
--     (и trans-цепочка, и локальный snapshot), в shared pgstat temp не
--     флашится вовсе - то есть счетчик, переживающий commit, недоступен.
...
-- Позитивный контроль: размер не менялся через commit -> стата ОСТАЕТСЯ
...
-- Размер вырастает в разы, а relid не попадает в touched-список.
...
-- ----------------------------------------------------------------------
-- 30. use_typanalyze=off (Haas-Stokes): чрезмерно широкие varlena НЕ
--     детостятся - иначе полный TOAST-payload материализуется в памяти
--     и на multi-MB значениях x sample_rows раздувает backend до OOM.
--     Зеркалим ядерный WIDTH_THRESHOLD (1024 байта): широкие значения
--     считаются toowide (assumed distinct), а не детостятся и сравниваются.
--     Наблюдаемо: набор ОДИНАКОВЫХ широких значений не схлопывается в
--     n_distinct=1 (что потребовало бы детоста всех) - stadistinct < 0
--     (все distinct), как у обычного ANALYZE.
-- ----------------------------------------------------------------------
CREATE TEMP TABLE t_wide (doc text);
INSERT INTO t_wide SELECT repeat('x', 4000) FROM generate_series(1, 300);
BEGIN;
SET LOCAL fasttrun.use_typanalyze = off;
SELECT fasttrun_collect_stats('t_wide');
SELECT stadistinct < 0 AS wide_not_collapsed
  FROM fasttrun_inspect_stats('t_wide');
COMMIT;
DROP TABLE t_wide;
...
--      идет block-sampling, а не полный O(таблицы) скан.
-- ----------------------------------------------------------------------
...
--     измеряется напрямую, reltuples берется как верхняя граница
--     (totalrows) вместо устаревшего заниженного значения.  Проверяем
--     через fasttrun_relstats -- planner-visible срез relstats из
--     session-local кэша.
-- ----------------------------------------------------------------------
CREATE TEMP TABLE t_partial_zero (id int, flag bool);
INSERT INTO t_partial_zero SELECT g, false FROM generate_series(1, 100000) g;
CREATE INDEX t_partial_zero_idx ON t_partial_zero (id) WHERE flag;
SELECT fasttrun_analyze('t_partial_zero');

-- Индекс стартует пустым (предикат не матчит ни одной строки).
SELECT reltuples::bigint = 0 AS partial_zero_start
  FROM fasttrun_relstats('t_partial_zero_idx');

-- Предикат начинает матчить половину строк.
UPDATE t_partial_zero SET flag = true WHERE id <= 50000;

-- Без выборки relpages обновляется (физический замер), reltuples берется
-- как верхняя граница totalrows, а не остается устаревшим нулем.
SET fasttrun.sample_rows = 0;
SELECT fasttrun_analyze('t_partial_zero');
SELECT relpages > 1 AS relpages_refreshed,
       reltuples::bigint = 100000 AS reltuples_upper_bound
  FROM fasttrun_relstats('t_partial_zero_idx');
...
--     block-sample DEBUG-маркер, а 2000 строк < sample_rows делают выборку
...
-- ----------------------------------------------------------------------
-- 34. Обычный ANALYZE со списком колонок передает ядру только
--     перечисленные колонки. Для b сохраняется локальная статистика,
--     полный ANALYZE передает ядру обе колонки.
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- 36. VACUUM (ANALYZE) со списком колонок ведет себя как ANALYZE,
--     а вызов без списка передает ядру все колонки.
-- ----------------------------------------------------------------------
...
--     статистика всех затронутых таблиц остается скрытой.
-- ----------------------------------------------------------------------
...
--     appendrel-планировании. Ядерная оценка переживает отклоненные вызовы.
-- ----------------------------------------------------------------------
...
-- Batch прерывается на родителе (контракт bulk), а обычная таблица
...
-- Ядерная inherited-стата не спрятана отклоненными вызовами.
DO $$
DECLARE ln text; est int := NULL;
BEGIN
  FOR ln IN EXPLAIN SELECT x FROM t_inh_stats_parent GROUP BY x LOOP
...
--     скрываться, планировщик берет дефолтные оценки до ядерного
...
-- ----------------------------------------------------------------------
-- 44. Емкость: fasttrun_cache_stats() и fasttrun.max_stats_memory.
--     Сверх бюджета таблица без кешированной column-статы остается без
--     нее (WARNING один раз на бэкенд на авто-пути, NOTICE на каждый
...
-- Узкая таблица A: кэш еще под бюджетом, стата собирается.
...
FROM fasttrun_cache_stats();
-- Ровно текущий размер разрешен, поэтому первая новая таблица может превысить порог.
...
-- Статистика A остается видимой: уникальный столбец -> rows=1.
DO $$
DECLARE ln text; est int := NULL;
BEGIN
...
--     pgstat считает tuples_updated по отношению целиком, поэтому без учета
...
--     статистика обязана прятаться, как до появления поколоночного учета.
...
-- 46j. Пересбор, пропускающий ранее собранную колонку: json берет только
--      полный сборщик, облегченному нужен btree-компаратор.
...
-- 46m. Первый пересбор отношения, за которым расширение еще не следит, изнутри
--      действующего UPDATE. Учет начатых операторов ведется и для таких
...
--      таблица-наследник, и ее начатый UPDATE обязан считаться так же.
...
-- 46o. Поздний триггер и завершенная транзакция. Признак снимает сам оператор
--      CREATE TRIGGER, заблаговременно: после COMMIT счетчиков уже нет, и
...
-- 46p. Рабочий цикл целиком, три раза подряд: учет начатых операторов не должен
...
--      транзакцию. Отношение, покидающее кэш, обязано ее освобождать: иначе
...
/* отношение покидает кэш только на COMMIT: до него передача ядру лишь меняет
   правило, а запись остается на месте, и маску снял бы очередной пересбор */
...
--      Счетчики привязаны к транзакции, поэтому накопление живет внутри нее.
...
-- 46s. INSERT ... ON CONFLICT DO UPDATE - отдельный вход в учет колонок:
...
--      триггер, но доверие не возвращает: потеря идет в безопасную сторону и
...
BEGIN
  /* делитель зависит от строки, поэтому ошибка происходит уже в исполнении:
     свертка констант сняла бы оператор до старта и проверять было бы нечего */
...
-- 46w. Обратная сторона: подтранзакция, отмененная внутри действующего UPDATE,
...
--      переписал таблицу. Скан идет по снимку начала команды и новых версий не
--      видит, а счетчики рядом уже включают эти записи, поэтому сравнивать
--      образец не с чем: он не отдается до следующего чистого пересбора.
...
-- 46y. Обратная сторона учета по подтранзакциям: вложенный UPDATE того же
--      отношения упал и был перехвачен, внешний завершился штатно. Записи о
--      начатых операторах не остается, поэтому следующий пересбор снова точен.
...
-- 46z. Реестр начатых операторов конечен. Оператор, у которого временных целей
--      больше, чем слотов, переводит весь реестр в осторожный ответ до конца
--      транзакции, а ее граница возвращает обычную работу. Постоянные цели в
--      реестр не попадают, см. 46aj.
...
-- 46ab. Учет по колонкам решает "называли или нет", а не "сколько строк по
--       этой колонке". Пока колонку не называли, чужие UPDATE ее не старят.
...
--       возвращает образец прошлого периода вместе с его запретом, а сам UPDATE
...
BEGIN;
/* Запрещенный образец: пересбор идет из выходной части запроса, чей CTE уже
   переписал таблицу. Счетчики рядом с образцом включают эти записи, поэтому
...
/* Откат возвращает запрещенный образец прошлого периода. Пометка периода тут
...
--       облегченному сборщику нужен btree-компаратор, а json его не имеет.
...
BEGIN;
/* Запрещенный образец json снимается тем же способом -- изнутри запроса, чей
...
SELECT count(*) > 0 AS cte_nulled_json, fasttrun_collect_stats('t_seq_skip_unsafe')
  FROM w;
/* Облегченному сборщику нужен btree-компаратор, а у json его нет: новый период
   эту колонку пропускает, и запрещенный образец остается на месте. */
SET LOCAL fasttrun.use_typanalyze = off;
...
--       выборки, идущей по снимку начала команды, поэтому в учет начатых
...
-- 46af. Писатель может успеть завершиться, а выборка все равно останется на
...
--       завершен, снимок анализа взят после него, и статистика обязана жить.
BEGIN;
...
--       идущий писатель отдельно: выборка внутри выражения COPY идет по снимку
--       той же команды и уже вставленных строк не видит, а счетчики их держат.
...
-- 46aj. Постоянные цели в реестр начатых операторов не попадают: статистику
--       fasttrun собирает только для временных таблиц. UPDATE секционированной
--       таблицы без отсечения секций перечисляет в целях каждую секцию, здесь
--       их 100. Если бы они занимали слоты, реестр переполнился бы, как в 46z,
--       и сбор до конца транзакции не доходил бы до планировщика. Секции
--       чередуются, UNLOGGED и обычные, чтобы проверить любую невременную цель.
CREATE TABLE t_seq_perm_parts (k int, v int) PARTITION BY HASH (k);
DO $pp$
DECLARE i int;
BEGIN
  FOR i IN 0..99 LOOP
    EXECUTE format('CREATE %s TABLE t_seq_perm_parts_%s PARTITION OF t_seq_perm_parts '
                   'FOR VALUES WITH (MODULUS 100, REMAINDER %s)',
                   CASE WHEN i % 2 = 0 THEN 'UNLOGGED' ELSE '' END, i, i);
  END LOOP;
END$pp$;
INSERT INTO t_seq_perm_parts SELECT g, 0 FROM generate_series(1, 1000) g;
CREATE TEMP TABLE t_seq_perm_probe (id int, note int);
INSERT INTO t_seq_perm_probe SELECT g, 0 FROM generate_series(1, 100000) g;
SELECT fasttrun_collect_stats('t_seq_perm_probe');
BEGIN;
UPDATE t_seq_perm_parts SET v = 1 WHERE k <> 5;
SELECT fasttrun_collect_stats('t_seq_perm_probe');
SELECT ft_rows_est('SELECT * FROM t_seq_perm_probe WHERE id = 1', 'on t_seq_perm_probe')
       <= 10 AS perm_partitions_update_keeps_stats;
COMMIT;

-- 46ak. То же для DELETE и для рабочего цикла целиком: очистка, заполнение и
--       fasttrun_analyze в транзакции, где уже прошел DELETE по всем секциям.
BEGIN;
DELETE FROM t_seq_perm_parts WHERE k <> 5 AND v = -1;
SELECT fasttruncate('t_seq_perm_probe');
INSERT INTO t_seq_perm_probe SELECT g, 0 FROM generate_series(1, 100000) g;
SELECT fasttrun_analyze('t_seq_perm_probe');
SELECT ft_rows_est('SELECT * FROM t_seq_perm_probe WHERE id = 1', 'on t_seq_perm_probe')
       <= 10 AS perm_partitions_delete_keeps_stats;
COMMIT;

-- 46al. COPY в постоянную таблицу тоже не занимает слот. Шестнадцать временных
--       целей реестр заполняют, но не переполняют, это проверяется первым.
--       Постоянная цель COPY, под которой триггер пишет те же шестнадцать,
--       была бы семнадцатой.
CREATE TABLE t_seq_perm_copy (x int);
CREATE TEMP TABLE t_seq_sixteen (id int);
DO $st$
DECLARE i int;
BEGIN
  FOR i IN 1..15 LOOP
    EXECUTE format('CREATE TEMP TABLE t_seq_sixteen_c%s () INHERITS (t_seq_sixteen)', i);
  END LOOP;
END$st$;
CREATE FUNCTION t_seq_perm_copy_trg() RETURNS trigger LANGUAGE plpgsql AS $ct$
BEGIN
  UPDATE t_seq_sixteen SET id = id;
  RETURN NEW;
END$ct$;
CREATE TRIGGER t_seq_perm_copy_trg BEFORE INSERT ON t_seq_perm_copy
  FOR EACH ROW EXECUTE FUNCTION t_seq_perm_copy_trg();
BEGIN;
UPDATE t_seq_sixteen SET id = id;
SELECT fasttrun_collect_stats('t_seq_perm_probe');
SELECT ft_rows_est('SELECT * FROM t_seq_perm_probe WHERE id = 1', 'on t_seq_perm_probe')
       <= 10 AS sixteen_temp_targets_fit;
COMMIT;
BEGIN;
COPY t_seq_perm_copy FROM STDIN;
1
\.
SELECT fasttrun_collect_stats('t_seq_perm_probe');
SELECT ft_rows_est('SELECT * FROM t_seq_perm_probe WHERE id = 1', 'on t_seq_perm_probe')
       <= 10 AS perm_copy_target_not_registered;
COMMIT;
DROP TABLE t_seq_perm_copy;
DROP FUNCTION t_seq_perm_copy_trg();
DO $st$
DECLARE i int;
BEGIN
  FOR i IN 1..15 LOOP
    EXECUTE format('DROP TABLE t_seq_sixteen_c%s', i);
  END LOOP;
END$st$;
DROP TABLE t_seq_sixteen;
DROP TABLE t_seq_perm_probe;
DROP TABLE t_seq_perm_parts;

-- 46am. Фильтр смотрит на каждую цель оператора, а не на названное отношение.
--       Временный наследник постоянного родителя - цель UPDATE этого родителя,
--       и пересбор наследника изнутри такого UPDATE обязан считаться идущим под
--       писателем. Условию не отвечает ни одна строка: износа от UPDATE нет, и
--       скрыть образец может только реестр.
CREATE TABLE t_seq_perm_parent (id int, note int);
CREATE TEMP TABLE t_seq_temp_child () INHERITS (t_seq_perm_parent);
INSERT INTO t_seq_temp_child SELECT g, 0 FROM generate_series(1, 100000) g;
BEGIN;
UPDATE t_seq_perm_parent SET note = 1
  WHERE id = (SELECT -1 FROM fasttrun_collect_stats('t_seq_temp_child'));
SELECT ft_rows_est('SELECT * FROM t_seq_temp_child WHERE id = 1', 'on t_seq_temp_child')
       >= 100 AS temp_child_of_permanent_parent_registered;
ROLLBACK;
DROP TABLE t_seq_temp_child;
DROP TABLE t_seq_perm_parent;
