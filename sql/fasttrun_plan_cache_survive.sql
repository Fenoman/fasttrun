...
-- ----------------------------------------------------------------------
-- 11. ROLLBACK TO SAVEPOINT после fasttruncate инвалидирует локальный
--     plan cache РОВНО РАЗ на уникальный relid, а не по разу на индекс.
--     fasttruncate трогает heap + каждый индекс + toast как отдельные
--     relid'ы; empty-storage ветка abort-колбэка инвалидировала owning
--     heap на КАЖДОЙ индексной entry -> ~2N walk'ов на N индексов.
--     Дедуп через inval_seen: N уникальных relid = N DEBUG-строк.
--     Здесь heap + btree + hash + gin + toast + toast-idx = 6 уникальных.
-- ----------------------------------------------------------------------
...
-- выше уже отработал на warning-уровне). Ожидаем 6 DEBUG-строк -- по одной
-- на уникальный relid, а не 2N+1.
SET client_min_messages = debug1;
