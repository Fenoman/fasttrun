...
-- ----------------------------------------------------------------------
-- 5. Проверка fasttrun.zero_sinval_truncate.
--    on очищает файлы через unlink и smgrcreate, off вызывает
--    RelationTruncate для каждого отношения. Оба способа должны давать
--    одинаковый результат, в том числе после переключения настройки.
--    Общие сообщения инвалидации проверяются отдельным скриптом.
-- ----------------------------------------------------------------------
...
-- Заполняем ~5 MB данных (50k строк x 100 байт)
INSERT INTO t_disk
...
-- ----------------------------------------------------------------------
-- 12. fasttrun_analyze_bulk(VARIADIC text[]): batch wrapper над
--     fasttrun_analyze для нескольких temp таблиц за один вызов.
--     Каждая обрабатывается ровно как одиночный fasttrun_analyze;
--     plan-cache invalidations идут inline на каждой таблице, но
--     первая помечает планы is_valid=false и rest short-circuit'ятся
--     в core's PlanCacheRelCallback.
-- ----------------------------------------------------------------------
CREATE TEMP TABLE t_bulk_a (id int);
CREATE TEMP TABLE t_bulk_b (id int);
CREATE TEMP TABLE t_bulk_c (id int);
INSERT INTO t_bulk_a SELECT generate_series(1, 100);
INSERT INTO t_bulk_b SELECT generate_series(1, 200);
INSERT INTO t_bulk_c SELECT generate_series(1, 300);

-- Positional (variadic): same args, no ARRAY wrapper.
SELECT fasttrun_analyze_bulk('t_bulk_a', 't_bulk_b', 't_bulk_c');
-- fasttrun publishes stats into rd_rel (process-local), NOT on-disk
-- pg_class -- read via fasttrun_relstats().
SELECT 't_bulk_a' AS t, (reltuples = 100::real) AS ok FROM fasttrun_relstats('t_bulk_a')
UNION ALL
SELECT 't_bulk_b', (reltuples = 200::real) FROM fasttrun_relstats('t_bulk_b')
UNION ALL
SELECT 't_bulk_c', (reltuples = 300::real) FROM fasttrun_relstats('t_bulk_c')
ORDER BY t;

-- VARIADIC ARRAY[] form: programmatic call sites.
INSERT INTO t_bulk_a SELECT generate_series(1, 50);    -- now 150 rows
INSERT INTO t_bulk_b SELECT generate_series(1, 50);    -- now 250 rows
SELECT fasttrun_analyze_bulk(VARIADIC ARRAY['t_bulk_a','t_bulk_b']);
SELECT 't_bulk_a' AS t, (reltuples = 150::real) AS ok FROM fasttrun_relstats('t_bulk_a')
UNION ALL
SELECT 't_bulk_b', (reltuples = 250::real) FROM fasttrun_relstats('t_bulk_b')
ORDER BY t;

-- NULL and missing names are silently skipped -- t_bulk_c stats unchanged.
SELECT fasttrun_analyze_bulk(VARIADIC ARRAY['t_bulk_c', NULL, 'no_such_relation']);
SELECT reltuples = 300::real AS c_unchanged FROM fasttrun_relstats('t_bulk_c');

-- Non-temp / non-heap raises ERROR.
CREATE TEMP TABLE t_bulk_part (a int) PARTITION BY RANGE (a);
SELECT fasttrun_analyze_bulk('t_bulk_a', 't_bulk_part');
DROP TABLE t_bulk_part;
DROP TABLE t_bulk_a, t_bulk_b, t_bulk_c;

-- ----------------------------------------------------------------------
-- 13. fasttrun.invalidate_threshold: проверяем что GUC принимается и
--     fasttrun_analyze не падает ни на дефолте (0.2), ни на 0 (legacy
--     behaviour).  Корректность распределения invalidations через
--     PlanCacheRelCallback проверяется в scripts/check_fasttrun_bulk_
--     analyze_overhead.sh (bpftrace, требует sudo + Linux).
-- ----------------------------------------------------------------------
CREATE TEMP TABLE t_thresh (id int, payload text);
INSERT INTO t_thresh SELECT g, md5(g::text) FROM generate_series(1, 1000) g;
SELECT fasttrun_analyze('t_thresh');

SET fasttrun.invalidate_threshold = 0.2;
INSERT INTO t_thresh SELECT g, md5(g::text) FROM generate_series(1, 10) g;
SELECT fasttrun_analyze('t_thresh');

SET fasttrun.invalidate_threshold = 0;
INSERT INTO t_thresh SELECT g, md5(g::text) FROM generate_series(1, 10) g;
SELECT fasttrun_analyze('t_thresh');
SELECT reltuples BETWEEN 1000::real AND 1100::real AS within_band
  FROM fasttrun_relstats('t_thresh');
RESET fasttrun.invalidate_threshold;
DROP TABLE t_thresh;

-- ----------------------------------------------------------------------
-- 14. Предварительно опустошённый heap (DELETE + VACUUM) с раздутым
--     индексом.  RelationGetNumberOfBlocks считает только heap main
--     fork; после VACUUM он равен 0, но btree-индекс VACUUM физически
--     не ужимает -- его storage остаётся раздутым.  fasttruncate должен
--     всё равно сбросить индекс (как обычный TRUNCATE), а не
--     превратиться в no-op из-за пустого heap.
-- ----------------------------------------------------------------------
CREATE TEMP TABLE t_bloat_idx (id int primary key, payload text);
INSERT INTO t_bloat_idx SELECT g, md5(g::text) FROM generate_series(1, 100000) g;
DELETE FROM t_bloat_idx;
VACUUM t_bloat_idx;

-- heap опустошён до 0 байт, индекс всё ещё раздут.
SELECT pg_relation_size('t_bloat_idx') = 0 AS heap_emptied,
       pg_relation_size('t_bloat_idx_pkey') > 1000000 AS idx_bloated;

SELECT fasttruncate('t_bloat_idx');

-- Индекс сброшен, несмотря на нулевой heap.
