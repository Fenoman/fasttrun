--
-- fasttrun_migration — полная проверка пути обновления 2.0 -> latest.
--
...
-- ----------------------------------------------------------------------
-- 6. Проверяем цепочку 2.1 -> latest, включая пустые SQL-only
--    апгрейды релизов, где менялся только .so/документация/тесты.
-- ----------------------------------------------------------------------
ALTER EXTENSION fasttrun UPDATE;

DO $$
BEGIN
    IF (SELECT extversion FROM pg_extension WHERE extname = 'fasttrun') <> '2.3.1' THEN
        RAISE EXCEPTION 'fasttrun extension was not updated to 2.3.1';
    END IF;
END
$$;

-- ----------------------------------------------------------------------
-- 7. После DROP EXTENSION все функции должны исчезнуть
