...
-- relstats и column stats переживают COMMIT, но xact-local delta
-- state очищается, и следующий analyze должен заново просканировать
-- heap перед новым delta-hit циклом.
--
-- Жесткие пороги внутри DO-блока (срабатывают только при регрессии):
--   * теплый analyze (попадание через дельту) не медленнее 5 мс
--   * полный reservoir refresh после массового DML не
--     медленнее 2000 мс (строгий предел по медиане - MAX_REFRESH_MS
--     в scripts/check_fasttrun_perf_smoke.sh)
--   * ускорение теплый/холодный не менее 20x
--   * fasttruncate миллионной таблицы не медленнее 500 мс (на быстром
--     железе это единицы-десятки мс, а строгий SLO с настраиваемым
--     порогом живет в scripts/check_fasttrun_perf_smoke.sh)
...
-- - обычные локальные значения: ускорение 1000x+, доли мс на теплый
-- вызов, ~50-150 мс на refresh.  При срабатывании в исключение
...
-- ----------------------------------------------------------------------
-- Сам бенчмарк.  Все внутри одного DO-блока, потому что кэш ленивого
-- режима живет внутри одной транзакции и сбрасывается callback'ом
-- границы транзакции на ее закрытии.
-- ----------------------------------------------------------------------
DO $$
DECLARE
    t_start         timestamptz;
...
BEGIN
    ...
    -- статистики через полный reservoir sample (дельта-математика
    -- обновляет число строк, refresh обновляет n_distinct / null_frac
    -- / width).  Второй analyze идет чистым попаданием в кэш через
    ...
    RAISE NOTICE 'after DELETE 500k: 1st=% ms (full refresh), 2nd=% ms (pure delta)',
                 round(t_after_d, 4), round(t_after_d_warm, 4);

    /* Полный пересбор на 1M строк x 50 колонок после DELETE и после UPDATE
     * занимает 180-330 мс на Apple M-серии и до 670 мс на сервере с 2 vCPU
     * и PostgreSQL 18. 2000 мс - порог регрессии, а не SLO: строгий предел
     * по медиане нескольких прогонов проверяет check_fasttrun_perf_smoke.sh
     * (MAX_REFRESH_MS). */
    IF t_after_d > 2000 THEN
        RAISE EXCEPTION 'full refresh after DELETE 500k too slow: % ms',
                        round(t_after_d, 4);
    END IF;
    ...
    -- порога: первый analyze -> полный reservoir refresh,
    ...
    RAISE NOTICE 'after UPDATE 500k: 1st=% ms (full refresh), 2nd=% ms (pure delta)',
                 round(t_after_u, 4), round(t_after_u_warm, 4);

    IF t_after_u > 2000 THEN
        RAISE EXCEPTION 'full refresh after UPDATE 500k too slow: % ms',
                        round(t_after_u, 4);
    END IF;
    ...
    -- смешанная пара.  DELETE 1000 строк из 500000 (0.2%) НЕ должен
    -- стоить полного сканирования.  Дельта-математика обрабатывает
    -- его за те же ~30 микросекунд, что и пустой проход с дельтой = 0.
    --
    -- На входе: 500000 строк (после UPDATE), все с нечетным id
    --           (1, 3, 5, ..., 999999), потому что предыдущий
    --           DELETE id%2=0 уничтожил все четные.
    -- ----------------------------------------------------------

    -- Маленький DELETE: убираем 1000 строк (нечетные id 1, 3, ..., 1999).
    ...
    -- DELETE убирает нечетные id 2001, 2003, ..., 2199 = 100 строк.
    INSERT INTO t_bench (id)
    ...
    IF t_trunc > 500 THEN
        RAISE EXCEPTION 'fasttruncate too slow on 1M-row table: % ms (expected <500)',
                        round(t_trunc, 2);
    END IF;
...
-- ----------------------------------------------------------------------
-- 9. Холодный проход только для счетчика строк (auto_collect_stats=off):
--    регрессионная защита чистого пути подсчета строк от накладных
--    расходов проверки свежести.  Должен быть в разы быстрее
--    холодного прохода со сбором статистики.
-- ----------------------------------------------------------------------
DO $$
DECLARE
    t_start timestamptz;
...
--     вызов RelationIdGetRelation и чтение pgstat (один раз на
--     отношение за планирование) плюс сравнение в каждом обращении
--     планировщика к статистике.  Меряем построение 1000 планов
...
BEGIN
    ...
    -- Жесткий порог на накладные расходы: даже на медленных CI
...
-- ----------------------------------------------------------------------
-- 11. Реальный продакшн-цикл: имитирует типичный сценарий тяжелого
--     PL/pgSQL-расчета, где в одной транзакции на одну временную
...
BEGIN
    ...
    -- Шаг 5: analyze (срабатывает полный reservoir refresh)
    t_step := clock_timestamp();
    PERFORM fasttrun_analyze('t_prod');
    ...
    -- cached stats будут скрыты до реального refresh)
    INSERT INTO t_prod
    ...
    -- Шаг 10: analyze (полный reservoir refresh)
    t_step := clock_timestamp();
    PERFORM fasttrun_analyze('t_prod');
    ...
    -- Жесткие пороги защиты от регрессии.  Каждый пересбор на узкой
    -- 100k-row таблице должен укладываться в 100 мс с большим
    -- запасом, а весь сценарий - в пару секунд (большая часть времени
...
-- ----------------------------------------------------------------------
-- 12. Переиспользование сессии пулером: клиент A создает временную
--     таблицу на 50k строк, прогоняет базовый цикл analyze, делает
--     COMMIT.  Затем "клиент B", получивший ту же самую сессию из
--     пула, делает fasttruncate и заполняет таблицу ДРУГИМ объемом
...
--
--     fasttruncate должен явно сбросить оба кэша таблицы: COMMIT
--     этого не делает, relstats и статистика колонок его переживают,
--     а callback границы транзакции сбрасывает только дельту и
--     опорный снимок счетчиков.
-- ----------------------------------------------------------------------
...
BEGIN
    ...
    -- проход + попадания через дельту, затем COMMIT приведет к
    -- срабатыванию callback'a границы транзакции, который сбросит
    -- дельту и опорный снимок счетчиков.
    ...
    -- Несколько раундов analyze + DML, все внутри транзакции клиента A
    INSERT INTO t_pool
...
-- транзакции сбрасывает дельту, а relstats и статистика колонок
-- клиента A остаются в кэшах.

-- Клиент B: получает сессию из пула.  Таблица все еще существует
...
BEGIN
    ...
    -- Клиент B заполняет таблицу НАМНОГО большим объемом данных
    ...
    -- засеял analyze cache опорным снимком pgstat счетчиков, поэтому
    -- этот вызов идет по delta-hit + full refresh, а не по
    -- холодному пути.  Главное - никакого протекания
...
--     (облегченный путь Haas-Stokes - только n_distinct/null_frac/width).
--     Жесткий порог: typanalyze не должна быть больше чем в 5x
--     медленнее, чем Haas-Stokes - иначе trade-off "качество планов
--     vs latency сбора" перестает быть приемлемым.
-- ----------------------------------------------------------------------
DO $$
DECLARE
    t_start    timestamptz;
...
BEGIN
    ...
    -- fasttruncate+refill, следующий analyze пойдет через refresh
    -- с полным сбором.
...
--     с дорогими compute_stats per-column, а на 4-col table обычно
...
BEGIN
    ...
    /* auto при default_statistics_target=100 берет ~30000 строк ->
     * reservoir в ~10x больше. На 4-col таблице ratio обычно 5-15x.
     * Жесткий потолок 30x ловит реальную регрессию без дребезга. */
    IF ratio > 30.0 THEN
