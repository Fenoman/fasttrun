...
-- Поэтому в таком сеансе поколоночный учёт UPDATE не работает вовсе — износ
-- снова считается по таблице целиком, как до его появления. Один раз за сеанс
-- об этом пишется в серверный лог.
--
...
-- своё соединение, поэтому проверки идут в новом: расширение уже стоит,
-- библиотека ещё нет, и её загрузит первый же вызов.
--
...
-- Носитель, ради которого учёт и выключается: пересбор изнутри уже идущего
...
-- Тот же носитель, но пишущий оператор вложен в блок DO и идёт через SPI.
...
-- Учёт по колонкам выключен: UPDATE служебной колонки гасит и ключевую.
