...
# транзакции, которая трогала временные объекты. Поэтому до PREPARE доходит
# только оператор с постоянными целями: здесь UPDATE родителя наследования с 20
# постоянными потомками, 21 цель - больше, чем слотов. Он не должен ни
# переполнить реестр, ни оставить в нем след после PREPARE TRANSACTION.
# Своего COMMIT-callback у этого пути нет, и сброс реестра на PREPARE служит
# второй линией защиты: проверка падает, только если сломаны обе.
#
