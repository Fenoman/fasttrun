...
статистики или счётчиков DML, а также незавершённая очистка возвращают
вызов на полный путь. Одного `reltuples = 0` для пропуска недостаточно.
После COMMIT быстрый путь восстанавливает базу счётчиков для следующего
заполнения и `fasttrun_analyze()`. Если пустое состояние ещё не согласовано
с сохранёнными планами, выполняется полная очистка с инвалидацией.
...
`UPDATE` учитывается по колонкам. pgstat считает изменённые строки по таблице
целиком, поэтому обновление одной служебной колонки прежде обесценивало
статистику всех остальных. Теперь износ от `UPDATE` относится только к колонкам,
названным в `SET`; `INSERT` и `DELETE` — по-прежнему ко всем сразу.

Учёт требует предзагрузки: цели оператора видны только при его старте, а хуки
...
Без предзагрузки учёт по колонкам выключен целиком, износ снова относится ко всем
колонкам сразу, и один раз за сеанс об этом пишется в серверный лог. Прежнее
поведение возвращается при этом **не полностью**: образец, снятый пока таблицу
пишут, не отдаётся планировщику ни в одном из режимов, пока его не заменит
новый. Сравнивать его не с чем — выборка идёт по снимку своей команды, а
счётчики рядом уже включают эти записи.

Для отдельной таблицы учёт отключается до следующего пересбора, когда списку
колонок нельзя доверять: есть триггеры или stored generated column, `UPDATE`
пришёл к наследнику через родителя, либо статистика колонки старше последнего
пересбора.

Список берётся из целей оператора, а не из изменённых значений: `UPDATE` без
единой совпавшей строки колонку всё равно отметит, и дальше ей засчитывается
...
счётчиков транзакции, поэтому несколько подпороговых транзакций подряд в сумме
...
расширение удаляет её запись незавершённой операции вместе с локальными
...
`2.3.0`, `2.3.1`, `2.3.2`, `2.3.3`, `2.3.4` и `2.4.0`.
...
В 2.4.1 исправлены утечки памяти при откате создания временных таблиц и
ускорена повторная очистка уже пустой таблицы. Переход 2.4.0 → 2.4.1 не
меняет SQL-объекты; исправления требуют новой C-библиотеки.

В 2.5.0 появился учёт износа статистики по колонкам: `UPDATE` служебной колонки
больше не обесценивает статистику ключей соединения. Переход 2.4.1 → 2.5.0 не
меняет SQL-объекты, но **требует изменения конфигурации**:

```
session_preload_libraries = 'fasttrun'
```

Без этой строки расширение работает как прежде, по таблице целиком, и один раз
за сеанс пишет об этом в серверный лог. Причина в том, что список колонок
оператора виден только при его старте, а хуки ставятся при загрузке библиотеки:
если она грузится по первому вызову, операторы, начавшиеся раньше, восстановить
уже нельзя.
...
- Учёт `UPDATE` по колонкам, однажды снятый с таблицы, возвращается только
  следующим пересбором: если триггер создали внутри savepoint и откатили его,
  учёт для этой таблицы остаётся приостановленным.
...
на одиночных прогонах разброс машины легко выдаёт за разницу между сборками то,
чего нет.
...
Журнал транзакций учитывает уровень создания временного отношения. При
откате этого уровня записи исчезающей таблицы и её индексов освобождаются;
пустые файлы ещё не удалённой ядром таблицы не удерживают её кеш в памяти.
Очистка проходит только по отношениям, отмеченным в завершающемся уровне.
