...
* **все индексы, toast-таблицу и её индекс** — порядок несущий: сначала
  все отношения открываются (сбой здесь оставляет таблицу нетронутой),
  затем фаза 1 — очистка storage всех индексов и toast (unlink +
  `smgrcreate`; heap ещё цел), затем очищается сам heap — последняя
  разрушительная операция — и только после этого фаза 2 пересоздаёт
  пустые структуры через `ambuild` (metapage btree, hash и т.д.).
  Ни одно состояние сбоя не сочетает пустой heap с индексом, где
  остались старые TID: до очистки heap данные целы, после неё все
  индексы уже пусты, и обращение к индексу без metapage даёт явную
  ошибку чтения, а не тихо неверные результаты (WARNING подсказывает
  пересоздать таблицу);
