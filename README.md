...
* **все индексы, toast-таблицу и её индекс** — в две фазы: сначала
  очищается storage всех сразу (unlink + `smgrcreate`), затем пустые
  структуры пересоздаются через `ambuild` (metapage btree, hash и т.д.).
  Сбой на любом шаге оставляет в худшем случае пустой индекс без
  metapage: первое же обращение к нему даёт явную ошибку чтения, а не
  тихо неверные результаты по старым TID (WARNING подсказывает
  пересоздать таблицу);
...
* **Top-level `ROLLBACK` не делает catalog-like undo для `rd_rel`** — savepoint paths обрабатываются локально, но при откате всей транзакции значения могут жить в relcache до следующего `fasttrun_analyze`, `fasttruncate` или reconnect. Это паритет с ядром: обычный `ANALYZE` тоже пишет `relpages`/`reltuples` in-place и не откатывает их на ROLLBACK — relation-level статистика в PostgreSQL нетранзакционна by design.
