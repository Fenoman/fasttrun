...
(см. "Статистика столбцов"). Специального положения fasttrun в списке библиотек
нет.
...
библиотеку после перехода тоже придется с рестартом (раздел "Обновление и
удаление"). Общий реестр учета `CREATE TEMP TABLE` при этом создается, но запись
в него по умолчанию выключена и включается параметром
`fasttrun.track_temp_creates` (раздел "Учет и предварительное создание
таблиц"). Строку о выключенном
...
### Параметры учета и предварительного создания
...
попытки в него не записываются.
...
`2.3.0`, `2.3.1`, `2.3.2`, `2.3.3`, `2.3.4`, `2.4.0`, `2.4.1` и `2.5.0`.
...
Между версиями 2.4.0, 2.4.1, 2.5.0 и 2.5.1 SQL-объекты расширения не меняются,
поэтому обновление сводится к замене C-библиотеки и `ALTER EXTENSION ... UPDATE`.
Версиям 2.5 при этом **нужна предзагрузка библиотеки**. Если fasttrun уже есть
...
`fasttrun_prewarm()`, задайте `fasttrun.track_temp_creates = on` (раздел "Учет и
...
- Если оператор пишет больше 16 временных таблиц сразу (`UPDATE` или `DELETE`
  секционированной временной таблицы без отсечения секций, родителя
  наследования с потомками), до конца транзакции любая таблица считается
  записываемой: статистика, собранная в этой транзакции после такого
  оператора, не отдается планировщику, пока ее не заменит новый сбор.
  Постоянные таблицы в этот счет не входят.
...
Цель `check-perf-smoke` из этого набора проверяет время `fasttruncate` по
секундомеру и потому зависит от скорости машины. Замеров два, и пределы у них
разные: узкая таблица (1 млн строк, две колонки, один индекс) - 100 мс,
...
обе медианы. Цель требует Linux, `bpftrace` и `sudo`, поэтому на macOS не
запускается.
...
поэтому их порядок не определяется положением fasttrun в
`shared_preload_libraries`.
